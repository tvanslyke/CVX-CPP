...
#ifndef EXPRESSIONS_SUBEXPRESSION_H_
#define EXPRESSIONS_SUBEXPRESSION_H_
...
class SubExpression: public FunctionExpression<SubExpression<Left, Right>>
{
public:
	...
	SubExpression(const left_type & left, const right_type & right):
...
template <class Left, class Right>
SubExpression<derived_type_t<Left>, derived_type_t<Right>> make_sub_expression(Left && left, Right && right)
{
	return SubExpression<derived_type_t<Left>, derived_type_t<Right>>(forward_as_derived(left), forward_as_derived(right));
}
} /* namespace cvx */

#endif /* EXPRESSIONS_SUBEXPRESSION_H_ */
